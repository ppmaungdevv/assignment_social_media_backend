...
import { auth_urls } from '../urls/auth.js';

const router = express.Router();

router.post(auth_urls.register, validate(registrationValidator), async (req, res) => {
  const data = await register(req.body)
  res.formattedResponse(data, 'User Registartion')
})

router.post(auth_urls.login, validate(loginValidator), async (req, res) => {
  const data = await login(req.body)
  return res.formattedResponse(data, 'Success Login')
})

router.post(auth_urls.refresh_token, async (req, res) => {
  const data = await refershToken(req)
  res.formattedResponse(data, 'Success Refresh')
})

router.post(auth_urls.logout, async (req, res) => {
