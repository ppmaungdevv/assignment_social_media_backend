...
export const getAccessTokenData = async ( payload, ttl = '1h' ) => {
  payload.jti = uuidv4()
  const token = jwt.sign(
    payload,
    process.env.JWT_SECRET,
    { expiresIn: process.env.ATOKEN_LIFESPAN_MINS || ttl }
  )
...
export const getRefreshTokenData = async () => {
  ...
  expires_at.setDate(expires_at.getDate() + process.env.RTOKEN_LIFESPAN_DAYS).toISOString
